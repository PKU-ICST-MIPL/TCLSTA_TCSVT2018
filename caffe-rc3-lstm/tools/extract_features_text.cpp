...
int feature_extraction_pipeline(int argc, char** argv) {
  ...
  CHECK_EQ(num_features,1)<<"exact_feature_text of this version takes only 1 blob name as input";
  std::string featurePath = save_feature_dataset_names + ".txt" ;
  CHECK(!std::ifstream(featurePath.c_str()))<<"file ["<<featurePath<<"] already exists.";
  std::ofstream outfile(featurePath.c_str()) ;
  Datum datum;
