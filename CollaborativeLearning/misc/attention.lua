...
function attention.alternating_atten(input_size_ques, input_size_img, embedding_size, ques_seq_size, img_seq_size)
    local inputs = {}
    local outputs = {}
    print(input_size_ques.."_"..input_size_img.."_"..embedding_size.."_"..ques_seq_size.."_"..img_seq_size)
    ...
    local ques_embed_dim = nn.Linear(input_size_ques, embedding_size)(nn.View(-1, input_size_ques)(ques_feat))--300*26,512
    local ques_embed = nn.View(-1, ques_seq_size, embedding_size)(ques_embed_dim)--300,26,512
    
    local feat = nn.Dropout(0.5)(nn.Tanh()(ques_embed))
    local h1 = nn.Linear(embedding_size, 1)(nn.View(-1, embedding_size)(feat))--300*26,1
    local P1 = nn.maskSoftMax()({nn.View(-1, ques_seq_size)(h1),mask})--300,26
    local ques_atten = nn.View(1,-1):setNumInputDims(1)(P1)--300,1,26
    local quesAtt1 = nn.MM(false, false)({ques_atten, ques_feat})--300,1,512
    local ques_atten_feat_1 = nn.View(-1, input_size_ques)(quesAtt1)--300,512

    -- img attention
    local ques_embed_img = nn.Linear(input_size_ques, embedding_size)(ques_atten_feat_1)--300,512

    local img_embed_dim = nn.Linear(input_size_img, embedding_size)(nn.View(-1, input_size_img)(img_feat))--300*196,512   

    local img_embed = nn.View(-1, img_seq_size, embedding_size)(img_embed_dim)--300,196,512

    local ques_replicate = nn.Replicate(img_seq_size,2)(ques_embed_img)--300,196,512
    
    local feat = nn.Dropout(0.5)(nn.Tanh()(nn.CAddTable()({img_embed, ques_replicate})))--300,196,512
    local h2 = nn.Linear(embedding_size, 1)(nn.View(-1, embedding_size)(feat))--300*196,1
    local P2 = nn.SoftMax()(nn.View(-1, img_seq_size)(h2))--300,196
    local img_atten = nn.View(1,-1):setNumInputDims(1)(P2)--300,1,196
    local visAtt = nn.MM(false, false)({img_atten, img_feat})--300,1,512
    local img_atten_feat = nn.View(-1, input_size_img)(visAtt)--300,512

    -- question attention

    local img_embed = nn.Linear(input_size_img, embedding_size)(img_atten_feat)--300,512
    local img_replicate = nn.Replicate(ques_seq_size,2)(img_embed)--300,26,512

    local ques_embed_dim = nn.Linear(input_size_ques, embedding_size)(nn.View(-1, input_size_ques)(ques_feat))--300*26,512
    local ques_embed = nn.View(-1, ques_seq_size, embedding_size)(ques_embed_dim)--300,26,512

    local feat = nn.Dropout(0.5)(nn.Tanh()(nn.CAddTable()({ques_embed, img_replicate})))--300,26,512

    local h3 = nn.Linear(embedding_size, 1)(nn.View(-1, embedding_size)(feat))--300*26,1
    local P3 = nn.maskSoftMax()({nn.View(-1, ques_seq_size)(h3),mask})--300,26
    local probs3dim = nn.View(1,-1):setNumInputDims(1)(P3)--300,1,26
    local quesAtt = nn.MM(false, false)({probs3dim, ques_feat})--300,1,512
    local ques_atten_feat = nn.View(-1, 2048)(quesAtt)--300,512
...
function attention.recursive_atten(input_size, embedding_size, last_embed_size, output_size)
    ...
    local outfeat = nn.Linear(last_embed_size, output_size)(nn.Dropout(0.5)(hidden1))
