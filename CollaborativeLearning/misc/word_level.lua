...
function layer:__init(opt)
    ...
    if self.atten_type == 'Alternating' then
      self.atten = attention.alternating_atten(self.hidden_size, self.hidden_size, self.hidden_size, self.seq_length, 25)   
    elseif self.atten_type == 'Parallel' then
      self.atten = attention.parallel_atten(self.hidden_size, self.hidden_size, self.hidden_size, self.seq_length, 25)   
    else
    ...
    elseif self.feature_type == 'Residual' then
      self.cnn = nn.Sequential()
                      ...
                      :add(nn.View(-1, 25, self.hidden_size))
                      :add(nn.Tanh())
                      :add(nn.Dropout(0.5))
      self.cnnStream = nn.Sequential()
                      :add(nn.View(2048):setNumInputDims(2))
...
function layer:getModulesList()
    return {self.cnnStream, self.atten, self.cnn}
end

function layer:parameters()
    local p1,g1 = self.cnn:parameters()

    local p2,g2 = self.cnnStream:parameters()
...
function layer:updateOutput(input)
  ...
  self.mask:resize(seq:size(1),seq:size(2)):zero()
  --self.mask[torch.eq(seq, 0)] = 1


  self.img_feat = self.cnn:forward(img)

  self.embed_output = self.cnnStream:forward(seq)

  --前两个为atten后特征,这个定义好的网络没有显示写出forward函数？
...
function layer:updateGradInput(input, gradOutput)
  ...
  local d_embed_ques, d_embed_img, dummy = unpack(self.atten:backward({self.embed_output, self.img_feat, self.mask}, {gradOutput[1], gradOutput[2]}))

  -- d_embed_ques:add(gradOutput[1])

  local dummy = self.cnnStream:backward(seq, d_embed_ques)

  -- d_embed_img:add(gradOutput[2])
